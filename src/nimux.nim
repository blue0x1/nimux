...
proc adcsHttpNtlmRequest(host: string; port, timeoutMs: int;
  ...
  try:
    ...
    proc u32le(v: int): string =
      result = newString(4); result[0] = char(v and 0xff); result[1] = char((v shr 8) and 0xff)
      result[2] = char((v shr 16) and 0xff); result[3] = char((v shr 24) and 0xff)
    ...
    let type1Flags = int(0xE2088237'u32)
    var type1 = "NTLMSSP\x00" & u32le(1) & u32le(type1Flags)
    type1.add u16le(0) & u16le(0) & u32le(40)
    type1.add u16le(0) & u16le(0) & u32le(40)
    ...
    let micOffset = 72
    let payloadOff = 88
    proc secbuf(len, off: int): string = u16le(len) & u16le(len) & u32le(off)
    var off = payloadOff
    ...
    var type3 = "NTLMSSP\x00" & u32le(3)
    type3.add lmSec & ntSec & domSec & usrSec & wsSec & mkSec & u32le(int(type3Flags))
