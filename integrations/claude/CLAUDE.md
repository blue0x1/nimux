...
- 1.0.5 added `nimux --version`, SMB `--spider`, broader `--interesting` SMB filename patterns, and anonymous/null-session `nimux smb --shares` enumeration when the target permits it.
...
Claude should verify local help before using 1.0.3/1.0.4/1.0.5 ADCS, LDAP capture, web discovery, and SMB spider flags. Common forms are:
